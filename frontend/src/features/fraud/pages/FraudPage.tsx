...
export default function FraudPage() {
  ...
  return (
    <div className="p-6 lg:p-8">
      <PageHeader
        title="Fraud Monitor"
        subtitle="5-rule detection: velocity, amount spikes, location anomalies, duplicates, night patterns"
      />
                    ...
                    <tr>
                      <td colSpan={8}>
                        <EmptyState
                          ...
                          description="Alerts appear automatically when suspicious patterns are detected. Your transactions are being monitored in real time."
                        />
                      </td>
                    </tr>
                  )
                  : alerts.map((alert) => (
                    <tr key={alert.id} className="border-b border-slate-50 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-700/30">
                      <td className="py-3 px-4 font-medium text-slate-700 dark:text-slate-300">{ALERT_TYPE_LABELS[alert.alert_type] ?? alert.alert_type}</td>
                      <td className="py-3 px-4">
                        <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${SEVERITY_COLORS[alert.severity] ?? ""}`}>{alert.severity}</span>
                      </td>
                      <td className="py-3 px-4 font-medium text-slate-900 dark:text-slate-100">{alert.transaction_amount ? formatCurrency(alert.transaction_amount) : "—"}</td>
                      <td className="py-3 px-4 text-slate-500 dark:text-slate-400 text-xs">{alert.account_number ?? "—"}</td>
                      <td className="py-3 px-4 text-slate-600 dark:text-slate-400 text-xs max-w-xs truncate">{alert.description ?? "—"}</td>
                      <td className="py-3 px-4">
                      ...
                      <td className="py-3 px-4 text-slate-400 dark:text-slate-500 text-xs whitespace-nowrap">{formatDateTime(alert.created_at)}</td>
                      <td className="py-3 px-4">
                        {alert.is_resolved ? (
